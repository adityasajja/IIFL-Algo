...
def test_going_live_requires_a_reason(api):
    ...
    with pytest.raises(HTTPException) as exc:
        api.set_execution_mode("live", reason="", principal=_OPERATOR)
    assert exc.value.status_code == 400
    # And the refusal must not have flipped the mode as a side effect.
    assert api.get_execution_mode()["mode"] == "paper"

    with pytest.raises(HTTPException):
        api.set_execution_mode("live", reason="   ", principal=_OPERATOR)  # whitespace is not a reason

    assert api.get_execution_mode()["mode"] == "paper"


def test_going_live_with_a_reason_succeeds_and_opens_the_gate(api):
    api.set_execution_mode("live", reason="3 weeks of paper fills matched the forecast", principal=_OPERATOR)
...
def test_returning_to_paper_needs_no_reason(api):
    api.set_execution_mode("live", reason="testing", principal=_OPERATOR)
    api.set_execution_mode("paper", reason="", principal=_OPERATOR)
    assert api.get_execution_mode()["mode"] == "paper"
...
def test_unknown_mode_is_rejected(api):
    ...
    for bad in ("", "LIVE", "test", "real"):
        with pytest.raises(HTTPException):
            api.set_execution_mode(bad, reason="x", principal=_OPERATOR)
    assert api.get_execution_mode()["mode"] == "paper"
...
def test_mode_changes_are_recorded(api):
    api.set_execution_mode("live", reason="first live session", principal=_ADITYA)
    api.set_execution_mode("paper", reason="done for the day", principal=_ADITYA)

    entries = api.get_audit(limit=200)["entries"]
...
def test_kill_switch_is_recorded(api):
    api.kill_switch(True, reason="halting into the RBI policy announcement", principal=_OPERATOR)
    api.kill_switch(False, reason="announcement digested, spreads normal", principal=_OPERATOR)
...
def test_the_kill_switch_requires_a_reason(api):
    ...
    for engaged in (True, False):
        with pytest.raises(HTTPException) as exc:
            api.kill_switch(engaged, reason="   ", principal=_OPERATOR)
        assert exc.value.status_code == 400
...
def test_trail_is_append_only(api):
    api.set_execution_mode("live", reason="a", principal=_OPERATOR)
    first = api.get_audit(limit=200)["entries"]
    api.set_execution_mode("paper", reason="b", principal=_OPERATOR)
    second = api.get_audit(limit=200)["entries"]
...
def test_trail_is_jsonl(api, tmp_path):
    api.set_execution_mode("live", reason="check the format", principal=_OPERATOR)
...
def test_missing_audit_file_reads_as_empty(api, tmp_path):
    assert api.get_audit(limit=200)["entries"] == []
